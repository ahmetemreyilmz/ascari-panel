...
const StatusBadge = ({ status, label, onClick }) => {
  ...
  return (
    <span 
      onClick={onClick} 
      className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${colors[status] || colors.draft} ${onClick ? 'cursor-pointer hover:opacity-80' : ''}`}
    >
...
export default function AscariDashboard() {
  ...
  const handleLogout = () => { 
    ...
    setQuickCart([]); 
    setQuickOfferDetails(null); 
  };
  
  const addToQuickCart = useCallback((product) => { 
    setQuickCart(prev => { 
  ...
  const createQuickOffer = () => { 
    ...
    setQuickOfferDetails({ 
      ...
      items: [...quickCart], 
      total: total, tax: total * 0.20, grandTotal: total * 1.20 
    }); 
  };
  ...
  const handleTicketSubmit = (e) => { 
    e.preventDefault(); 
    const newTicket = { id: `SRV-2023-00${data.tickets.length + 1}`, ...newTicketData, status: 'new', date: new Date().toISOString().split('T')[0], notes: '' }; 
    setData(prev => ({ ...prev, tickets: [newTicket, ...prev.tickets] })); 
    setShowNewTicketForm(false); 
    setNewTicketData({ customer: '', product: '', issue: '', priority: 'medium' }); 
  };
      ...
      return (
        <div className="space-y-6 animate-fadeIn">
          <div className="flex items-center">
            <button onClick={() => setSelectedCustomer(null)} className="mr-3 p-2 rounded-full hover:bg-slate-100 text-slate-500">
              <ArrowLeft className="w-5 h-5" />
            </button>
            <div>
              <h2 className="text-xl font-bold text-slate-900">{selectedCustomer.name}</h2>
              <p className="text-xs text-slate-500">Müşteri Kartı</p>
            </div>
          </div>
             ...
             <div className="lg:col-span-1 space-y-6">
                <div className="bg-white shadow rounded-lg overflow-hidden border-t-4 border-indigo-500">
                  <div className="p-6 text-center border-b border-slate-100">
                    <div className="mx-auto h-24 w-24 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 text-3xl font-bold mb-4">{selectedCustomer.name.charAt(0)}</div>
                    <h3 className="text-lg font-bold text-slate-900">{selectedCustomer.name}</h3>
                    <span className={`inline-flex mt-2 px-2.5 py-0.5 rounded-full text-xs font-medium ${selectedCustomer.type === 'company' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>{selectedCustomer.type === 'company' ? 'Kurumsal' : 'Bireysel'}</span>
                  </div>
                  <div className="px-6 py-4 bg-slate-50">
                    <div className="flex justify-between items-center mb-2"><span className="text-sm text-slate-500">Bakiye Durumu</span></div>
                    <div className={`text-2xl font-bold ${selectedCustomer.balance < 0 ? 'text-red-600' : 'text-green-600'}`}>{formatCurrency(selectedCustomer.balance)}</div>
                    <p className="text-xs text-slate-400 mt-1">{selectedCustomer.balance < 0 ? 'Ödeme Bekleniyor' : 'Güvenli'}</p>
                  </div>
                  <div className="p-6 space-y-3">
                    <div className="flex items-center text-sm text-slate-600"><Mail className="w-4 h-4 mr-3 text-slate-400"/> {selectedCustomer.email}</div>
                    <div className="flex items-center text-sm text-slate-600"><Phone className="w-4 h-4 mr-3 text-slate-400"/> {selectedCustomer.phone}</div>
                    <div className="flex items-center text-sm text-slate-600"><MapPin className="w-4 h-4 mr-3 text-slate-400"/> {selectedCustomer.address}</div>
                  </div>
                </div>
             </div>
                ...
                <div className="bg-white shadow rounded-lg overflow-hidden min-h-[500px]">
                   <div className="border-b border-slate-200">
                     <nav className="flex -mb-px">
                       <button onClick={() => setCustomerDetailTab('orders')} className={`w-1/3 py-4 px-1 text-center border-b-2 font-medium text-sm ${customerDetailTab === 'orders' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}><ShoppingCart className="w-4 h-4 inline-block mr-2 mb-0.5"/> Siparişler ({cOrders.length})</button>
                   ...
                   <div className="p-4">
                      {customerDetailTab === 'orders' && (cOrders.length > 0 ? cOrders.map(o => (
                        <div key={o.id} className="border border-slate-200 rounded-lg p-4 mb-3 hover:bg-slate-50 flex justify-between items-center">
                          <div><div className="flex items-center space-x-2"><span className="font-bold text-indigo-600">{o.id}</span><span className="text-xs text-slate-400">• {o.date}</span></div><div className="text-sm text-slate-600 mt-1">{o.lines.map(l => l.product).join(', ')}</div></div>
                          <div className="text-right"><div className="font-bold text-slate-800">{formatCurrency(o.amount)}</div><StatusBadge status={o.status} label={o.statusLabel} /></div>
                        </div>
                      )) : <div className="text-center py-10 text-slate-400">Kayıt yok.</div>)}
                      
                      {customerDetailTab === 'invoices' && (cInvoices.length > 0 ? cInvoices.map(i => (
                        <div key={i.id} className="border border-slate-200 rounded-lg p-4 mb-3 hover:bg-slate-50 flex justify-between items-center">
                          <div><div className="font-bold text-slate-700">{i.id}</div><div className="text-xs text-slate-500">Ref: {i.ref} • {i.date}</div></div>
                          <div className="text-right"><div className="font-bold text-slate-900">{formatCurrency(i.amount)}</div><PaymentBadge state={i.payment_state} /></div>
                        </div>
                      )) : <div className="text-center py-10 text-slate-400">Kayıt yok.</div>)}
                      
                      {customerDetailTab === 'tickets' && (cTickets.length > 0 ? cTickets.map(t => (
                        <div key={t.id} className="border border-slate-200 rounded-lg p-4 mb-3 hover:bg-slate-50">
                          <div className="flex justify-between mb-2"><span className="font-bold text-slate-800">{t.product}</span><StatusBadge status={t.status} /></div>
                          <p className="text-sm text-slate-600">{t.issue}</p><div className="mt-2 text-xs text-slate-400 flex justify-between"><span>Kayıt: {t.id}</span><span>{t.date}</span></div>
                        </div>
                      )) : <div className="text-center py-10 text-slate-400">Kayıt yok.</div>)}
                   </div>
                </div>
    ...
    return (
      <div className="space-y-6 animate-fadeIn">
        <div className="flex justify-between items-center bg-white p-4 rounded-lg shadow-sm">
          <h3 className="text-lg font-medium text-slate-900">Müşteri Listesi</h3>
          <div className="flex space-x-2">
            <button onClick={()=>setCustomerBalanceFilter('all')} className={`px-3 py-1 text-xs rounded-full ${customerBalanceFilter === 'all' ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100'}`}>Tümü</button>
            <button onClick={()=>setCustomerBalanceFilter('debtor')} className={`px-3 py-1 text-xs rounded-full ${customerBalanceFilter === 'debtor' ? 'bg-red-100 text-red-700' : 'bg-slate-100'}`}>Borçlular</button>
            <button onClick={()=>setCustomerBalanceFilter('creditor')} className={`px-3 py-1 text-xs rounded-full ${customerBalanceFilter === 'creditor' ? 'bg-green-100 text-green-700' : 'bg-slate-100'}`}>Alacaklılar</button>
          </div>
        </div>
        <div className="bg-white shadow sm:rounded-lg overflow-hidden">
          <ul className="divide-y divide-slate-200">
            {filteredCustomers.map((customer) => (
              <li key={customer.id} onClick={() => setSelectedCustomer(customer)} className="hover:bg-indigo-50 cursor-pointer transition-colors px-4 py-4 flex justify-between items-center">
                <div className="flex items-center">
                  <div className="h-10 w-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-600 font-bold mr-4">{customer.name.charAt(0)}</div>
                  <div><p className="text-sm font-medium text-indigo-600">{customer.name}</p><p className="text-xs text-slate-500">{customer.phone}</p></div>
                </div>
                <div className={`text-sm font-bold ${customer.balance < 0 ? 'text-red-600' : 'text-green-600'}`}>{formatCurrency(customer.balance)}</div>
              </li>
            ))}
          </ul>
        </div>
  ...
  const renderProducts = () => { 
      ...
      return (
        <div className="space-y-6 animate-fadeIn">
          <div className="flex justify-between items-center">
            <button onClick={() => {setSelectedProduct(null); setIsEditingProduct(false);}} className="flex items-center text-slate-500 hover:text-slate-900"><ArrowLeft className="w-5 h-5 mr-2" /> Ürünlere Dön</button>
            <button onClick={() => setIsEditingProduct(!isEditingProduct)} className={`flex items-center px-4 py-2 rounded-md font-medium text-sm ${isEditingProduct ? 'bg-green-600 text-white' : 'bg-indigo-600 text-white'}`}>
              {isEditingProduct ? <><Save className="w-4 h-4 mr-2"/> Kaydet</> : <><Edit3 className="w-4 h-4 mr-2"/> Düzenle</>}
            </button>
          </div>
          <div className="bg-white shadow rounded-lg p-8 flex flex-col md:flex-row gap-8">
            <div className="w-full md:w-1/3 flex items-center justify-center bg-slate-50 rounded-lg h-64 border border-slate-200"><PackageSearch className="w-24 h-24 text-slate-300" /></div>
            <div className="w-full md:w-2/3 space-y-6">
              <div><label className="text-xs font-bold text-slate-400 uppercase">Ürün Adı</label>{isEditingProduct ? <input className="w-full border p-2 rounded mt-1" defaultValue={selectedProduct.name} /> : <h2 className="text-2xl font-bold text-slate-900">{selectedProduct.name}</h2>}</div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-xs font-bold text-slate-400 uppercase">Kod</label><p className="font-mono text-slate-700">{selectedProduct.default_code}</p></div>
                <div><label className="text-xs font-bold text-slate-400 uppercase">Kategori</label><p className="text-slate-700">{selectedProduct.category}</p></div>
                <div><label className="text-xs font-bold text-slate-400 uppercase">Satış Fiyatı</label>{isEditingProduct ? <input className="border p-1 rounded w-full" defaultValue={selectedProduct.list_price}/> : <p className="text-xl font-bold text-indigo-600">{formatCurrency(selectedProduct.list_price)}</p>}</div>
                <div><label className="text-xs font-bold text-slate-400 uppercase">Maliyet</label>{isEditingProduct ? <input className="border p-1 rounded w-full" defaultValue={selectedProduct.cost_price}/> : <p className="text-xl font-bold text-slate-500">{formatCurrency(selectedProduct.cost_price)}</p>}</div>
                <div><label className="text-xs font-bold text-slate-400 uppercase">Ölçüler</label><p className="text-slate-700">{selectedProduct.dimensions || '-'}</p></div>
                <div><label className="text-xs font-bold text-slate-400 uppercase">Ağırlık</label><p className="text-slate-700">{selectedProduct.weight || '-'}</p></div>
                <div className="col-span-2 bg-slate-50 p-4 rounded border border-slate-200"><label className="text-xs font-bold text-slate-400 uppercase">Stok Durumu</label><p className={`text-lg font-bold ${selectedProduct.qty_available > 0 ? 'text-green-600' : 'text-red-600'}`}>{selectedProduct.qty_available} Adet Mevcut</p></div>
              </div>
            </div>
          </div>
        </div>
    ...
    return (
      <div className="space-y-6 animate-fadeIn">
        <div className="bg-white shadow sm:rounded-lg p-4"><input type="text" className="block w-full pl-3 sm:text-sm border-slate-300 rounded-md py-3 border" placeholder="Ürün Ara..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-4">
          {filteredProducts.map(p => (
            <div key={p.id} onClick={() => setSelectedProduct(p)} className="bg-white shadow rounded-lg p-5 flex flex-col justify-between h-48 border border-slate-100 cursor-pointer hover:border-indigo-500 transition-colors">
              <h3 className="font-bold text-slate-800">{p.name}</h3>
              <div><p className="text-xs text-slate-400 mb-1">{p.default_code}</p><span className="text-indigo-600 font-bold text-lg">{formatCurrency(p.list_price)}</span></div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-lg">
            ...
            <div className="border-t border-slate-200 px-4 py-5 sm:px-6">
              <h4 className="font-medium mb-2">Müşteri: {selectedOrder.customer}</h4>
              <table className="min-w-full divide-y divide-slate-200">
                <thead className="bg-slate-50"><tr><th className="px-6 py-3 text-left text-xs font-medium text-slate-500">Ürün</th><th className="px-6 py-3 text-right text-xs font-medium text-slate-500">Tutar</th></tr></thead>
                <tbody>{selectedOrder.lines.map((l, i)=><tr key={i}><td className="px-6 py-4 text-sm">{l.product} x {l.qty}</td><td className="px-6 py-4 text-right text-sm font-bold">{formatCurrency(l.subtotal)}</td></tr>)}</tbody>
              </table>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-lg">
          <ul className="divide-y divide-slate-200">
            {filteredOrders.length > 0 ? filteredOrders.map((order) => (
              <li key={order.id} onClick={() => setSelectedOrder(order)} className="px-4 py-4 sm:px-6 hover:bg-slate-50 transition-colors cursor-pointer">
                <div className="grid grid-cols-12 gap-4 items-center">
                  <div className="col-span-2 font-medium text-indigo-600">{order.id}</div>
                  <div className="col-span-4 text-slate-900">{order.customer}</div>
                  <div className="col-span-2 text-slate-500">{order.date}</div>
                  <div className="col-span-2 text-slate-900 font-medium text-right">{formatCurrency(order.amount)}</div>
                  <div className="col-span-2 flex justify-center"><StatusBadge status={order.status} label={order.statusLabel} /></div>
                </div>
              </li>
  ...
  const renderHelpdesk = () => { 
    if (selectedTicket) { 
      return (
        <div className="space-y-6 animate-fadeIn">
          <button onClick={() => setSelectedTicket(null)} className="flex items-center text-slate-500 hover:text-slate-900 mb-4"><ArrowLeft className="w-5 h-5 mr-2" /> Listeye Dön</button>
          <div className="bg-white shadow rounded-lg p-6 border-l-4 border-indigo-500">
            <div className="flex justify-between items-start mb-6">
              <div><h2 className="text-xl font-bold text-slate-900">{selectedTicket.issue}</h2><p className="text-sm text-slate-500">Kayıt: {selectedTicket.id} • {selectedTicket.date}</p></div>
              <div className="flex flex-col items-end gap-2">
                <select className="border border-slate-300 rounded-md text-sm p-1" value={selectedTicket.status} onChange={(e) => { const updated = {...selectedTicket, status: e.target.value}; setSelectedTicket(updated); }}>
                  <option value="new">Yeni</option><option value="progress">İşlemde</option><option value="solved">Çözüldü</option>
                </select>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-6">
              <div className="bg-slate-50 p-4 rounded"><h4 className="font-bold text-xs uppercase text-slate-400 mb-2">Müşteri Bilgisi</h4><p className="font-medium text-slate-900">{selectedTicket.customer}</p></div>
              <div className="bg-slate-50 p-4 rounded"><h4 className="font-bold text-xs uppercase text-slate-400 mb-2">İlgili Ürün</h4><p className="font-medium text-slate-900">{selectedTicket.product}</p></div>
              <div className="col-span-2"><h4 className="font-bold text-xs uppercase text-slate-400 mb-2">Notlar</h4><textarea className="w-full border border-slate-300 rounded p-2 text-sm" rows="3" defaultValue={selectedTicket.notes}></textarea></div>
            </div>
          </div>
        </div>
      ) 
    ...
    return (
      <div className="space-y-6 animate-fadeIn">
        <div className="flex justify-between items-center"><h2 className="text-lg font-medium text-slate-900">Arıza Kayıtları</h2><button onClick={() => setShowNewTicketForm(true)} className="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm">Yeni Kayıt</button></div>
        {showNewTicketForm && (
          <div className="bg-white p-6 shadow-lg rounded-lg border border-indigo-100">
            <div className="flex justify-between mb-4"><h3 className="font-bold">Yeni Kayıt</h3><button onClick={()=>setShowNewTicketForm(false)}><X className="w-5 h-5"/></button></div>
            <form onSubmit={handleTicketSubmit} className="grid grid-cols-2 gap-4">
              <input placeholder="Müşteri" className="border p-2 rounded" required value={newTicketData.customer} onChange={e=>setNewTicketData({...newTicketData, customer:e.target.value})}/>
              <input placeholder="Ürün" className="border p-2 rounded" required value={newTicketData.product} onChange={e=>setNewTicketData({...newTicketData, product:e.target.value})}/>
              <textarea placeholder="Sorun" className="border p-2 rounded col-span-2" required value={newTicketData.issue} onChange={e=>setNewTicketData({...newTicketData, issue:e.target.value})}/>
              <button type="submit" className="bg-indigo-600 text-white p-2 rounded col-span-2">Kaydet</button>
            </form>
          </div>
        )}
        <div className="bg-white shadow overflow-hidden sm:rounded-lg">
          <ul className="divide-y divide-slate-200">
            {data.tickets.map(t=><li key={t.id} onClick={() => setSelectedTicket(t)} className="px-4 py-4 flex justify-between cursor-pointer hover:bg-slate-50"><div><div className="font-bold text-slate-800">{t.product}</div><div className="text-sm text-slate-500">{t.customer} - {t.issue}</div></div><StatusBadge status={t.status}/></li>)}
          </ul>
        </div>
      </div>
    ); 
  ...
  const renderQuickOfferFull = () => { 
    const filteredProducts = data.products.filter(product => { 
      const matchesSearch = product.name.toLowerCase().includes(searchTerm.toLowerCase()) || product.default_code.toLowerCase().includes(searchTerm.toLowerCase()); 
        ...
        <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
          <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10 border border-slate-200">
            <form className="space-y-6" onSubmit={handleConnect}>
              <div><label className="block text-sm font-medium text-slate-700">Odoo URL</label><input type="text" className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="https://sirketiniz.odoo.com" value={credentials.url} onChange={e => setCredentials({...credentials, url: e.target.value})} /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-slate-700">Veritabanı</label><input type="text" className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="db_name" value={credentials.db} onChange={e => setCredentials({...credentials, db: e.target.value})} /></div>
                <div><label className="block text-sm font-medium text-slate-700">Giriş Yetkisi</label><select value={userRole} onChange={(e) => setUserRole(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-slate-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"><option value="admin">Yönetici / Admin</option><option value="sales">Satış Personeli (Mağaza)</option></select></div>
              </div>
              <div><label className="block text-sm font-medium text-slate-700">E-Posta / Kullanıcı</label><input type="text" className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value={credentials.username} onChange={e => setCredentials({...credentials, username: e.target.value})} /></div>
              <div><label className="block text-sm font-medium text-slate-700">Şifre</label><input type="password" className="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value={credentials.password} onChange={e => setCredentials({...credentials, password: e.target.value})} /></div>
              <button type="submit" disabled={loading} className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-slate-900 hover:bg-slate-800 transition-colors">{loading ? <span className="flex items-center"><RefreshCw className="animate-spin -ml-1 mr-2 h-4 w-4" /> Bağlanıyor...</span> : 'Bağlantı Kur'}</button>
            </form>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col w-64">
          <div className="flex flex-col h-0 flex-1 bg-slate-900">
            <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
              <div className="flex items-center flex-shrink-0 px-4 mb-8 h-10">{!logoError ? <img src="Ascari Beyaz Logo.png" alt="Ascari" className="h-10 w-auto" onError={() => setLogoError(true)} /> : <span className="text-white font-bold text-2xl">ASCARI</span>}</div>
              <div className="px-4 mb-6 flex items-center"><div className="bg-slate-800 p-2 rounded-full mr-3">{userRole === 'admin' ? <Shield className="text-yellow-400 w-5 h-5"/> : <User className="text-indigo-400 w-5 h-5"/>}</div><div><div className="text-white text-sm font-bold">{userRole === 'admin' ? 'Yönetici' : 'Satış Personeli'}</div><div className="text-slate-400 text-xs">Odoo Bağlantısı Aktif</div></div></div>
              <nav className="mt-2 flex-1 px-2 space-y-1">
                {menuItems.map((item) => (
                  <button key={item.id} onClick={() => { setActiveTab(item.id); setSelectedOrder(null); setFilterStatus('all'); setSearchTerm(''); setQuickCart([]); setQuickOfferDetails(null); setSelectedCustomer(null); }} className={`${activeTab === item.id ? 'bg-slate-800 text-white border-l-4 border-indigo-500' : 'text-slate-300 hover:bg-slate-800 hover:text-white'} group flex w-full items-center px-2 py-3 text-sm font-medium rounded-r-md transition-all`}>
                    <item.icon className={`${activeTab === item.id ? 'text-indigo-400' : 'text-slate-400 group-hover:text-slate-300'} mr-3 flex-shrink-0 h-5 w-5`} />{item.name}
                  </button>
                ))}
              </nav>
            </div>
            <div className="flex-shrink-0 flex bg-slate-800 p-4"><button onClick={()=>{setIsConnected(false);handleLogout()}} className="text-xs font-medium text-slate-300 hover:text-white flex items-center"><LogOut className="h-3 w-3 mr-1" /> Çıkış Yap</button></div>
          </div>
        </div>
      </div>
      <div className="flex-1 overflow-auto focus:outline-none">
        <main className="flex-1 relative pb-8 z-0 overflow-y-auto">
          <div className="bg-white shadow px-4 py-4 sm:px-6 lg:px-8 flex justify-between items-center sticky top-0 z-10">
            <h1 className="text-2xl font-bold text-slate-900">{activeTab === 'dashboard' ? 'Genel Bakış' : activeTab === 'sales' ? 'Satış Yönetimi' : activeTab === 'customers' ? 'Müşteri İlişkileri' : activeTab === 'accounting' ? 'Muhasebe' : activeTab === 'products' ? 'Ürün Kataloğu' : activeTab === 'helpdesk' ? 'Teknik Servis' : activeTab === 'quick_offer' ? 'Hızlı Teklif' : 'Geliştirici'}</h1>
            <div className="flex items-center space-x-4"><span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800"><span className="w-2 h-2 bg-green-500 rounded-full mr-1.5"></span>Odoo Bağlı</span></div>
          </div>
          <div className="mt-8 px-4 sm:px-6 lg:px-8">
            {activeTab === 'dashboard' && (
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
                {userRole === 'admin' ? (<div className="bg-white overflow-hidden shadow rounded-lg p-5"><div className="flex items-center"><div className="flex-shrink-0"><DollarSign className="h-6 w-6 text-slate-400" /></div><div className="ml-5 w-0 flex-1"><dt className="text-sm font-medium text-slate-500 truncate">Toplam Ciro</dt><dd className="text-lg font-medium text-slate-900">{formatCurrency(data.salesStats.totalRevenue)}</dd></div></div></div>) : null}
                <div className="bg-white overflow-hidden shadow rounded-lg p-5"><div className="flex items-center"><div className="flex-shrink-0"><FileText className="h-6 w-6 text-slate-400" /></div><div className="ml-5 w-0 flex-1"><dt className="text-sm font-medium text-slate-500 truncate">Aktif Teklifler</dt><dd className="text-lg font-medium text-slate-900">{data.salesStats.activeQuotations}</dd></div></div></div>
                <div className="bg-white overflow-hidden shadow rounded-lg p-5"><div className="flex items-center"><div className="flex-shrink-0"><Package className="h-6 w-6 text-slate-400" /></div><div className="ml-5 w-0 flex-1"><dt className="text-sm font-medium text-slate-500 truncate">Onaylı Siparişler</dt><dd className="text-lg font-medium text-slate-900">{data.salesStats.confirmedOrders}</dd></div></div></div>
              </div>
            )}
